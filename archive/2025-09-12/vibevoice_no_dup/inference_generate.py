...
def main() -> None:
    ...
    with torch.no_grad():
        outputs = model.generate(**inputs, max_new_tokens=1)  # generation triggers acoustic branch
        # Expect model to output audio via a decode method; fallback to model-specific API
        if hasattr(model, "decode_last_speech"):
            ...
            # As a placeholder, produce silence with requested duration
